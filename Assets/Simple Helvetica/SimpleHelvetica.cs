...
public class SimpleHelvetica : MonoBehaviour
{
	...
	[HideInInspector]
	public bool withGlobalCollider = true;
	...
	private BoxCollider globalCollider;
	...
	public void GenerateText() {
		...
		Bounds globalBounds = new Bounds() { center = transform.position, size = Vector3.zero };
		...
		for (int ctr = 0; ctr <= Text.Length - 1; ctr++) {
			...
			if (childObjectName != " ") {
				...
				LineHeight = Mathf.Max(LineHeight, bounds.size.y);
				//Debug.Log (bounds.size.x*ObjScale.x);
			} else {
				CharXLocation += SpaceWidth;
			}
			LineWidth = CharXLocation;
		}


		// center if requested
		if (center) {
			TextWidth = Mathf.Max(TextWidth, LineWidth - SpaceWidth);
			transform.localPosition = new Vector3( - TextWidth / 2f, transform.localPosition.y, transform.localPosition.z);
		}
		...
		transform.Find("_Alphabets").gameObject.SetActiveRecursively(false);
#endif
		if (withGlobalCollider)
			CreateGlobalCollider();
		else
			RemoveGlobalCollider();
	}

	private void CreateGlobalCollider() {
		RemoveGlobalCollider();

		Renderer[] letters = GetComponentsInChildren<Renderer>(false);
		if (letters.Length > 0) {
			globalCollider = gameObject.AddComponent<BoxCollider>();
			globalCollider.size = new Vector3(TextWidth, LineHeight - CharYLocation, 4f);
			globalCollider.center = new Vector3(TextWidth / 2f, LineHeight - globalCollider.size.y / 2f, globalCollider.size.z / 2f);
		}
	...
	void AddLetter(GameObject LetterObject) {
		...
		float newScaleX = NewLetter.transform.localScale.x / Mathf.Abs(ObjScale.x);
		float newScaleY = NewLetter.transform.localScale.y / Mathf.Abs(ObjScale.y);
		float newScaleZ = NewLetter.transform.localScale.z / Mathf.Abs(ObjScale.z);
		...
		CharXLocation += CharacterSpacing; //add a small space between letters
	}
	...
	void ResetText() {
		...
		//get object scale
		ObjScale = transform.InverseTransformVector(transform.localScale);
	...
	private void RemoveGlobalCollider() {
